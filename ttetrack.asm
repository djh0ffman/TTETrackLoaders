...
;-----------------------------------------------------------
;
; TTE Track - non-turbo edition
;
; A fast-ish byte loader with integrated ZX0 unpacker
;
...
; a0 = destination
; a1 = mfm buffer ( size is MFM_BUFSIZE )
;
;-----------------------------------------------------------
       incdir     include
       include    hw.i
...
MFM_BUFSIZE  equ (MFM_READLEN+2)*2
...
TTE_TurboRead:
       ...
       lea        -TTETrack_Sizeof(sp),sp
       move.l     sp,a5 

       move.w     td_currenttrack(pc),TTETrack_CurrentTrack(a5)
       move.w     #$4489,TTETrack_SyncMark(a5)
	...
	; disable DSKSYN & DSKBLK
       ...
       ; --------- td_motoron                                       ; select the drive, start motor
       move.b     (a3),d1
...
.waitdrive:  
       ...
       ; decode header
       move.w     TTETrack_SyncMark(a5),d0
       cmp.w      (a4)+,d0
       bne        .error
       move.l     #$55555555,d2
       move.l     (a4)+,d0
...
.startloading
       ...
       pea        zx0_decompress_tte(pc)
       move.l     (sp)+,TTETrack_UnpackState+(5*4)(a5)
...
.exit
       ...
       lea        td_currenttrack(pc),a0
       move.w     TTETrack_CurrentTrack(a5),(a0)
       lea        TTETrack_Sizeof(sp),sp
.wait
       ;move.w     #$00f,$dff180
       ;btst       #6,$bfe001
...
td_startdma:
	...
	; select MFM encoding and enable synchronization word (DSKSYNC)
       ...
       move.w     TTETrack_SyncMark(a5),DSKSYNC(a6)
...
td_sectors:
	; restore the first SYNC, which is not written by the DMA
       ...
       move.w     TTETrack_SyncMark(a5),d6
       move.w     d6,-(a0)
...
.4:    movem.l    (a0),d0-d1
	...
	; verify the track number
       ;swap       d0
       ;cmp.b      TTETrack_CurrentTrack+1(a5),d0
       ;bne        .err_wrongtrk
       ;swap       d0
